...
struct SplitByCharsepCallback
{
    ...
    {
        SplitByCharsepCallback& data = *static_cast<SplitByCharsepCallback*>(user_data);
        std::vector<std::string>& ref_result = *data.result_vec_ptr;
        const bool trimmed = data.trimmed;
        const bool allow_empty = data.allow_empty;
        const size_t max_return = data.max_return;
        std::string_view trim_chars = data.trim_chars;
        if (ref_result.size() >= max_return)
        {
            return false;
        }
        ...
        return ref_result.size() < max_return;
    }
}; // struct SplitByCharsepCallback
