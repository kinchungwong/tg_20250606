...
void split_by_charsep(
    ...
    bool (*callback)(const std::string_view&, const std::string_view&, void*),
    void* callback_data
)
{
    ...
    while (next < len)
    {
        ...
        if (end_nonsep == npos)
        {
             ...
             */
            std::string_view first_arg = input.substr(start_nonsep);
            std::string_view second_arg;
            callback(first_arg, second_arg, callback_data);
            return;
        }
        else if (end_sep == npos)
        {
            // The input string ends with one or more separator.
            std::string_view first_arg = input.substr(start_nonsep, end_nonsep - start_nonsep);
            std::string_view second_arg = input.substr(start_sep);
            callback(first_arg, second_arg, callback_data);
            return;
        }
        else
        {
            ...
            std::string_view first_arg = input.substr(start_nonsep, end_nonsep - start_nonsep);
            std::string_view second_arg = input.substr(start_sep, end_sep - start_sep);
            bool should_continue = callback(first_arg, second_arg, callback_data);
            if (!should_continue)
            {
                return;
            }
