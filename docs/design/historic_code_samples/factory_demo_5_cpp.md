# factory_demo_5.cpp (converted into markdown)
```cpp
...
using DataContextFunctor = std::function<void(DataContextPtr)>;
using DataContextCatchFunctor = std::function<std::optional<std::exception_ptr>(DataContextPtr)>;
...
{
public:
    InputContext(DataContextPtr ctx, const std::string& name);
...
{
public:
    OutputContext(DataContextPtr ctx, const std::string& name);
    ...
    T* operator->() const; // valid after emplace; must throw otherwise
    void emplace_on_first_use(bool allow = true); // if configured, deref on empty shared_ptr calls default emplace.
private:
    DataContextPtr m_ctx;
    std::string m_name; // name of the data in the context, possibly a local name (not namespaced)
    //! @todo Should store a fast retrieval key or slot ID for the data from the given m_ctx
...
void facade_demo()
{
    ...
    struct io_blur0
    {
        ...
        {
            omat_blur.emplace_on_first_use();
        }
    };

    auto f_blur0 = [](DataContextPtr ctx) -> void
    {
        auto io = std::make_shared<io_blur0>(ctx);
        if (ctx->is_exec_time())
        {
